...
def test_water():
    ...
    X_true = atom.get_repr(mol, ["H", "O"], 0, None, dm=None,
                      guess='LB', model='lowdin-long-x', auxbasis='ccpvdzjkfit', valence_only=False)
    assert(X.shape == X_true.shape)
    for a, a_true in zip(X, X_true):
        assert(a[0] == a_true[0])                        # atom type
        assert(not np.linalg.norm(a[1]-a_true[1]) < 1e-08)   # atom representations
